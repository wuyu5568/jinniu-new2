...
- `GET /api/admin_jinniu/deposit` 拉新充值；**无**进程内定时（本轮不加 cron）。
- 表 `eth_user_record`：**一合约序号一行**，`last` = 该序号，**`UNIQUE(last)`** 幂等。
- 游标：`MAX(last)`；空表视为 `-1`；下次从 `last+1` 处理到 `userLength-1`。
- 金额 **≥100**（硬编码）且地址已注册 → `status=success`，入账户余额并写 `ledger_entries` deposit。
- 未注册或 `<100` → **仍写行并推进**（C1）：`status=skipped`，`user_id=0`（未注册），地址放 `hash`，`type` 记原因（`unregistered` / `below_min`）。
